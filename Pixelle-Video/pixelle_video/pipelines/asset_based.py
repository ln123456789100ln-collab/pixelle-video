...
class AssetBasedPipeline(LinearVideoPipeline):
    ...
    # Pool of fitness-oriented templates, randomly selected per generation
    ...
    async def __call__(
        ...
        pre_analyzed_asset_index: Optional[Dict[str, Any]] = None,
        **kwargs
    ) -> PipelineContext:
        """
        ...
        Args:
            ...
            asset_descriptions: Optional dict mapping asset path to user-provided description
            ...
            pre_analyzed_asset_index: Pre-analyzed asset metadata (skips setup_environment)
        ...
        ctx = PipelineContext(
            ...
            params={
                "assets": assets,
                "asset_descriptions": asset_descriptions or {},
        ...
        try:
            ...
            if pre_analyzed_asset_index:
                # Skip analysis — use pre-analyzed data from batch pre-step
                self.asset_index = pre_analyzed_asset_index
                # Still need task dir setup
                task_dir, task_id = create_task_output_dir()
                ctx.task_id = task_id
                ctx.task_dir = Path(task_dir)
                ctx.final_video_path = get_task_final_video_path(task_id)
                ctx.asset_index = self.asset_index
            else:
    ...
    async def setup_environment(self, context: PipelineContext) -> PipelineContext:
        ...
        # Get user-provided asset descriptions (if any)
        user_descriptions: Dict[str, str] = context.request.get("asset_descriptions", {})
        ...
        for i, asset_path in enumerate(assets, 1):
            ...
            # Check if user provided a manual description for this asset
            if asset_path in user_descriptions and user_descriptions[asset_path].strip():
                description = user_descriptions[asset_path].strip()
                logger.info(f"📝 Using user description for {asset_path_obj.name}: {description[:50]}...")
            else:
                # No manual description, try AI analysis
                ...
                if asset_type == "image":
                    try:
                        ...
                        description = f"Image file: {asset_path_obj.name}"
                    ...
                    try:
                        ...
                        description = f"Video file: {asset_path_obj.name}"

                else:
                    logger.warning(f"Unknown asset type: {asset_path}")
                    description = f"Unknown asset: {asset_path_obj.name}"
    ...
    async def initialize_storyboard(self, context: PipelineContext) -> PipelineContext:
        ...
        # Randomly select a fitness-themed template for visual variety
        template_name = random.choice(self.FITNESS_TEMPLATE_POOL)
        ...
        # Build font import for Google Fonts
        font_name = context.params.get("font_name", "")
        font_import = ""
        if font_name and font_name.strip():
            font_slug = font_name.strip().replace(" ", "+")
            font_import = f'@import url("https://fonts.googleapis.com/css2?family={font_slug}:wght@400;500;600;700;800;900&display=swap");'

        template_params = {
            "font_name": font_name if font_name else "PingFang SC",
            "font_import": font_import,
            "text_color": context.params.get("text_color", "#ffffff"),
            "text_size": context.params.get("text_size", 48),
            "text_bg": "true" if context.params.get("text_bg") else "false",
            "text_stroke": "true" if context.params.get("text_stroke", True) else "false",
            **(context.params.get("template_params") or {})
        }
    ...
    async def produce_assets(self, context: PipelineContext) -> PipelineContext:
        ...
        for i, frame in enumerate(storyboard.frames, 1):
            ...
            # Add inter-scene pause (silent tail) so narration doesn't run
            # continuously without breathing room between scenes.
            inter_scene_pause = float(context.request.get("inter_scene_pause", 0.8))
            if inter_scene_pause > 0 and i < total_frames:
                current_audio = Path(frame.audio_path)
                padded_path = current_audio.parent / f"{current_audio.stem}_padded{current_audio.suffix}"
                subprocess.run([
                    'ffmpeg', '-i', str(current_audio),
                    '-af', f'apad=pad_dur={inter_scene_pause}',
                    '-y', str(padded_path)
                ], check=True, capture_output=True)
                frame.audio_path = str(padded_path)
                logger.debug(f"  Added {inter_scene_pause}s inter-scene pause")
    ...
    async def post_production(self, context: PipelineContext) -> PipelineContext:
        ...
        # Generate intro frame if enabled and title exists
        intro_enabled = context.request.get("intro_enabled", True)
        intro_duration = float(context.request.get("intro_duration", 1.5))

        if intro_enabled and context.title:
            intro_video = await self._create_intro_frame(context, intro_duration)
            if intro_video:
                scene_videos = [intro_video] + scene_videos
                logger.info(f"📺 Added intro frame ({intro_duration}s)")
        ...
        concat_transition = context.request.get("transition", "auto")
        self.core.video.concat_videos(
            videos=scene_videos,
            output=str(final_video_path),
            method="xfade",
            transition=concat_transition,
            bgm_path=bgm_path,
            bgm_volume=bgm_volume,
            bgm_mode=bgm_mode
        )
    ...
    async def _create_intro_frame(self, context: PipelineContext, duration: float = 1.5) -> Optional[str]:
        """
        Create a title intro frame at the beginning of the video.

        This gives BGM time to establish before narration starts,
        avoiding the disconnected feeling of voice+music appearing simultaneously.
        ...
        Returns:
            Path to intro video segment, or None if creation fails
        """
        import subprocess
        from pathlib import Path

        try:
            intro_dir = Path(context.task_dir) / "frames" / "00_intro"
            intro_dir.mkdir(parents=True, exist_ok=True)

            # 1. Render title-only HTML (dark background, big centered title)
            from pixelle_video.services.frame_html import HTMLFrameGenerator
            from pixelle_video.utils.template_util import resolve_template_path

            template_path = resolve_template_path(context.config.frame_template)
            generator = HTMLFrameGenerator(template_path)

            # Get template params for consistent styling
            tp = context.config.template_params or {}

            title_image_path = str(intro_dir / "title.png")
            await generator.generate_frame(
                title=context.title,
                text="",  # No narration on intro
                image="",  # No image, dark background
                ext={
                    "text_color": tp.get("text_color", "#ffffff"),
                    "text_size": tp.get("text_size", 48),
                    "text_bg": tp.get("text_bg", "false"),
                    "text_stroke": tp.get("text_stroke", "true"),
                },
                output_path=title_image_path
            )

            # 2. Generate short silent audio
            silent_audio = str(intro_dir / "silent.mp3")
            subprocess.run([
                'ffmpeg', '-f', 'lavfi',
            ...
            # 3. Create video segment from title image + silent audio
            intro_video = str(intro_dir / "intro.mp4")
            self.core.video.create_video_from_image(
                image=title_image_path,
                audio=silent_audio,
                output=intro_video,
                fps=context.config.video_fps
            )

            logger.info(f"✅ Intro frame created: {intro_video} ({duration}s)")
            return intro_video

        except Exception as e:
            logger.warning(f"Failed to create intro frame: {e}, skipping")
            return None
