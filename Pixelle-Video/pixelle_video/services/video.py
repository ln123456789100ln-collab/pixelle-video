...
class VideoService:
    ...
    # Pool of subtle, non-jarring transitions for auto mode cycling
    AUTO_TRANSITION_POOL = [
        "fade",
        "fadeblack",
        "fade",
        "dissolve",
    ...
    def concat_videos(
        ...
        videos: List[str],
        output: str,
        method: Literal["demuxer", "filter", "xfade"] = "xfade",
        transition: str = "auto",
        ...
        # Resolve transitions for xfade mode
        if method == "xfade":
            transitions = self._resolve_transitions(len(videos), transition)
        ...
        if bgm_path:
            ...
            if method == "xfade":
                concat_result = self._concat_xfade(videos, temp_output, duration=xfade_duration, transitions=transitions)
            elif method == "demuxer":
            ...
            if method == "xfade":
                return self._concat_xfade(videos, output, duration=xfade_duration, transitions=transitions)
            elif method == "demuxer":
    ...
    def _resolve_transitions(self, n_videos: int, transition: str) -> List[str]:
        """
        Resolve transition type(s) into a per-pair list.

        When transition is "auto", cycles through AUTO_TRANSITION_POOL for
        each adjacent video pair, giving varied but subtle transitions.

        Args:
            n_videos: Number of videos being concatenated
            transition: Transition mode — "auto", "none", or a specific type

        Returns:
            List of transition type strings, one per adjacent pair (len = n_videos - 1)
        """
        n_pairs = n_videos - 1
        if n_pairs <= 0:
            return []
        ...
        if transition == "auto":
            pool = self.AUTO_TRANSITION_POOL
            return [pool[i % len(pool)] for i in range(n_pairs)]
        elif transition == "none":
            return []
        else:
            return [transition] * n_pairs
    ...
    def _concat_xfade(
        ...
        Uses FFmpeg xfade filter (FFmpeg 4.1+) for true overlap crossfade.
        Each adjacent pair gets its own transition type and a correctly
        calculated offset so the transition lands at the scene boundary.

        Args:
            videos: List of video file paths
            output: Output video file path
            duration: Crossfade duration in seconds (default 0.8)
            transitions: Per-pair transition types (len = n_videos - 1).
        ...
        # Probe durations so we can calculate the right xfade offset
        # for each pair.  offset = accumulated_duration_so_far - xfade_duration
        clip_durations = []
        for v in videos:
            try:
                probe = ffmpeg.probe(v)
                clip_durations.append(float(probe['format']['duration']))
            except Exception:
                logger.warning(f"Could not probe {v}, assuming 0")
        ...
        filter_parts = []
        for i in range(n):
            filter_parts.append(f'[{i}:v]setpts=PTS-STARTPTS[{i}v]')
        for i in range(n):
            filter_parts.append(f'[{i}:a]asetpts=PTS-STARTPTS[{i}a]')

        # xfade chain — calculate offset per pair so the crossfade lands
        # at the END of the accumulated previous content.
        xfade_parts = []
        prev_label = '0v'
        accumulated = 0.0  # running duration of the previous xfade output
        for i in range(1, n):
            out_label = 'v' if i == n - 1 else f'xf{i}'
            t = transitions[i - 1]

            if i == 1:
                # First pair: offset = duration of clip 0 - xfade_duration
            ...
            # Update accumulated duration for this xfade output
            if i == 1:
                accumulated = clip_durations[0] + clip_durations[1] - duration
            else:
                accumulated = accumulated + clip_durations[i] - duration

            prev_label = out_label
        filter_parts.append(';'.join(xfade_parts))

        # acrossfade chain for audio
        audio_parts = []
        prev_label = '0a'
        for i in range(1, n):
            out_label = 'a' if i == n - 1 else f'af{i}'
            audio_parts.append(
                f'[{prev_label}][{i}a]acrossfade='
                f'd={duration}:c1=nofade:c2=nofade[{out_label}]'
            )
            prev_label = out_label
        ...
        try:
            ...
            trans_summary = ', '.join(transitions[:3])
            if len(transitions) > 3:
                trans_summary += '...'
            logger.success(
                f'Videos concatenated with xfade '
                f'({duration}s, [{trans_summary}]): {output}'
            )
            return output
        except subprocess.CalledProcessError as e:
            error_msg = e.stderr if e.stderr else str(e)
            logger.error(f'FFmpeg xfade error: {error_msg[:500]}')
            raise RuntimeError(
                f'Failed to concatenate with xfade: {error_msg[:200]}'
            )
